...
export default function SavedServices({
  ...
  const getCategoryLabel = (service: ESevaService) => {
    ...
    if (titleLower.includes("driving"))
      return language === "hi" ? "ड्राइविंग (Driving)" : "Driving";
    ...
    if (
      ...
      return language === "hi" ? "कृषि (Agriculture)" : "Agriculture";
    if (
      titleLower.includes("scholarship") ||
      titleLower.includes("education") ||
      titleLower.includes("school")
    )
      return language === "hi" ? "शिक्षा (Education)" : "Education";

    switch (service.category) {
      case "IDENTITY":
        return language === "hi" ? "पहचान पत्र (Identity)" : "Identity";
      case "FINANCE":
        return language === "hi" ? "वित्त (Finance)" : "Finance";
      case "HEALTH":
        return language === "hi" ? "स्वास्थ्य (Health)" : "Health";
      case "LABOUR":
        return language === "hi" ? "रोजगार (Labour)" : "Labour";
      case "LAND":
        return language === "hi" ? "राजस्व (Land)" : "Land";
      case "WELFARE":
        return language === "hi" ? "कल्याण (Welfare)" : "Welfare";
      default:
        return language === "hi" ? "अन्य (General)" : "General";
    }
  };
  ...
  const getCategoryStyles = (label: string) => {
    ...
    if (
      ...
      label.includes("यात्रा") ||
      label.includes("Driving") ||
      label.includes("ड्राइविंग") ||
      label.includes("Civic") ||
      label.includes("नागरिक") ||
      label.includes("Identity") ||
      label.includes("पहचान पत्र")
    ) {
      return {
        border: "border-orange-200/60 dark:border-orange-950/40",
    ...
    if (label.includes("Agriculture") || label.includes("कृषि")) {
      return {
        border: "border-emerald-200/60 dark:border-emerald-950/40",
    ...
    if (label.includes("Health") || label.includes("स्वास्थ्य")) {
      return {
        border: "border-cyan-200/60 dark:border-cyan-950/40",
        bg: "bg-cyan-50/40 dark:bg-cyan-950/10",
        text: "text-cyan-700 dark:text-cyan-400 font-bold",
        dot: "bg-cyan-550",
      };
    }
    if (label.includes("Labour") || label.includes("रोजगार")) {
      return {
        ...
        text: "text-amber-700 dark:text-amber-450 font-bold",
        dot: "bg-amber-550",
      };
    }
    if (label.includes("Land") || label.includes("राजस्व")) {
      return {
        border: "border-purple-200/60 dark:border-purple-950/40",
    ...
    return {
      border: "border-blue-200/60 dark:border-blue-950/40",
                ...
                Object.entries(groupedSavedServices).map(([categoryName, sList]) => {
                  ...
                  return (
                    <div
                      key={categoryName}
                      className="space-y-3"
                      id={`saved-group-${categoryName.toLowerCase().replace(/\s+/g, "-")}`}
                    >
                      {/* Sticky Group Header */}
                      <div className="flex items-center justify-between sticky top-0 py-1 bg-white dark:bg-slate-950 z-10 select-none border-b border-stone-100 dark:border-white/5">
                        <div className="flex items-center gap-1.5 pb-0.5">
                          ...
                          <span
                            className={`text-[10px] font-sans font-extrabold tracking-wider uppercase ${styles.text}`}
                          >
                            {categoryName}
                          </span>
                        </div>
                        <span className="text-[9px] font-mono text-stone-400 bg-stone-100 dark:bg-slate-900 px-1.5 py-0.5 rounded-sm font-semibold mb-0.5">
                          {sList.length}{" "}
                          {language === "hi" ? "सेवा" : sList.length === 1 ? "service" : "services"}
                        </span>
                      </div>
                      ...
                      <div className="space-y-4">
                        {sList.map((service) => {
                          const colors =
                            categoryColorMap[service.category] || categoryColorMap.WELFARE;
                          return (
                            <motion.div
                              layout
                              key={service.id}
                            ...
                            >
                              ...
                              <div className="flex items-center justify-between">
                                <span
                                  className={`text-[8.5px] ${colors.bg} ${colors.text} ${colors.border} border font-extrabold px-2 py-0.5 rounded-md font-mono uppercase tracking-wide`}
                                >
                                  {service.category}
                                </span>
                                ...
                                <div className="flex items-center gap-1">
                                  ...
                                  >
                                    <Languages
                                      className={`w-3.5 h-3.5 ${toggledTranslates[service.id] ? "text-orange-500 font-bold" : ""}`}
                                    />
                                  </button>
                              ...
                              <div className="space-y-1">
                                ...
                                <div className="pt-1 pb-1">
                                  <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-blue-50/70 border border-blue-200/40 text-[#1D4ED8] text-[9.5px] font-semibold font-mono">
                                    ...
                                    <span>
                                      {language === "hi"
                                        ? `औसत समय: ${service.processingTime}`
