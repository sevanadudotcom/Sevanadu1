...
export default function ESevaSupportChatbot({
  chatHistory,
  onSendMessage,
  onClearChat,
  loading,
}: ESevaSupportChatbotProps) {
  ...
  return (
    <div
      className="bg-white border border-gray-150 rounded-2xl flex flex-col h-[550px] shadow-sm overflow-hidden text-xs text-slate-800 animate-fade-in"
      id="suvidha-sahayak-bot"
    >
        ...
        {chatHistory.length === 0 && (
          <div className="text-center py-8 px-4 max-w-sm mx-auto space-y-4 select-none">
            ...
            <div className="space-y-1">
              ...
              <p className="text-[11px] text-slate-505 leading-relaxed">{t("chat.welcome_txt")}</p>
            </div>
          </div>
        )}
          ...
          return (
            ...
            >
              ...
              <div className="space-y-0.5">
                <div
                  className={`p-3 rounded-2xl border leading-relaxed font-sans text-[11.5px] whitespace-pre-line ${
                    isModel
                ...
                <span
                  className={`text-[8.5px] font-mono text-slate-455 block select-none ${isModel ? "text-left pl-1" : "text-right pr-1"}`}
                >
                  {msg.timestamp}
                </span>
              </div>
        ...
        {loading && (
          <div className="flex gap-3 max-w-[80%] mr-auto">
            ...
            <div className="p-3 bg-white border border-gray-250 rounded-2xl rounded-tl-none shadow-xs flex items-center gap-1.5 text-slate-500">
              ...
              ></span>
              <span className="font-mono text-[10.5px] text-slate-400 ml-1">
                {t("chat.thinking")}
              </span>
            </div>
          </div>
      ...
      <div className="p-4 bg-white border-t border-gray-150 shrink-0">
        <div className="flex items-center justify-between mb-3.5 px-1 select-none">
          ...
          >
            ...
            <span className="flex items-center justify-center w-3 h-3 text-[10px] font-black bg-gradient-to-r from-blue-500 via-red-500 to-yellow-500 bg-clip-text text-transparent select-none">
              G
            </span>
            <span>
              {language === "hi" ? "गूगल सर्च डेटा उपयोग करें" : "Use Google Search data"}
            </span>
            {useSearch && (
              <Sparkles className="w-3 h-3 text-amber-600 dark:text-amber-500 animate-pulse ml-0.5" />
            )}
          </button>
