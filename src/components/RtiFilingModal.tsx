...
export default function RtiFilingModal({
  ...
  const handleNextToPayment = (e: React.FormEvent) => {
    ...
    if (!auth) {
      triggerToast(
        isHi ? "कृपया सार्वजनिक प्राधिकरण चुनें।" : "Please select a Public Authority.",
        "error",
      );
      return;
    }
    if (!name || !email || !query) {
      triggerToast(
        isHi ? "कृपया सभी आवश्यक फ़ील्ड भरें।" : "Please fill in all mandatory fields.",
        "error",
      );
      return;
    }
    if (isBplExempt && !bplCardNo) {
      triggerToast(
        isHi ? "कृपया बीपीएल कार्ड संख्या दर्ज करें।" : "Please enter your BPL card number.",
        "error",
      );
      return;
    }
    ...
    if (isBplExempt) {
      ...
      triggerToast(
        isHi
          ? "RTI आवेदन सफलतापूर्वक दर्ज किया गया।"
          : "RTI Request submitted successfully under BPL category.",
        "success",
      );
    } else {
      setStep("payment");
  ...
  const handlePrintDraft = () => {
    printRtiDocument(
      "application",
      {
        auth: auth || "Public Authority (Central/State)",
        authName: getAuthName(auth),
        name: name.trim() || (isHi ? "आवेदक (नागरिक)" : "Applicant (Citizen)"),
        email: email.trim() || "citizen@gov.in",
        phone: phone.trim() || undefined,
        query:
          query.trim() ||
          (isHi
            ? "आरटीआई अधिनियम, 2005 की धारा 6(1) के अंतर्गत सार्वजनिक अभिलेखों का प्रकटीकरण।"
            : "Statutory disclosure of certified documents under Section 6(1) of the RTI Act, 2005."),
        feesPaid: isBplExempt ? 0 : 10,
        bplNo: isBplExempt ? bplCardNo || "BPL Verified" : undefined,
      },
      language,
      triggerToast,
    );
  ...
  const handlePrintReceipt = () => {
    ...
    printRtiDocument(
      ...
      {
        ...receipt,
        authName: getAuthName(receipt.auth),
        email,
        phone,
      },
      language,
      ...
      >
        ...
        <div className="p-5 border-b border-stone-150 dark:border-stone-850 flex items-center justify-between bg-stone-50 dark:bg-stone-900/60 shrink-0">
          ...
          <div className="flex items-center gap-2">
            <button
              type="button"
              onClick={step === "receipt" ? handlePrintReceipt : handlePrintDraft}
              className="flex items-center gap-1 px-2.5 py-1 text-[10.5px] font-bold text-stone-700 hover:text-stone-900 dark:text-stone-300 dark:hover:text-white bg-amber-500/10 hover:bg-amber-500/20 dark:bg-amber-400/10 dark:hover:bg-amber-400/20 border border-amber-500/25 rounded-lg transition cursor-pointer"
              title={
                step === "receipt"
                  ? isHi
                    ? "RTI पावती रसीद प्रिंट करें"
                    : "Print RTI Official Receipt"
                  : isHi
                    ? "RTI आवेदन प्रारूप प्रिंट करें"
                    : "Print RTI Application Form A"
              }
            >
              <Printer className="w-3.5 h-3.5 text-amber-600 dark:text-amber-400 shrink-0" />
              ...
              >
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="relative">
                    ...
                    >
                      <option value="">
                        {isHi ? "-- विभाग चुनें --" : "-- Select Department --"}
                      </option>
                    ...
                    <label className="flex items-center gap-2.5 p-3 scroll-py-1 bg-stone-50 dark:bg-stone-900 border border-stone-150 dark:border-stone-850 rounded-xl cursor-pointer select-none">
                      ...
                      <div>
                        <span className="text-[11px] font-bold text-stone-900 dark:text-white block">
                          {isHi
                            ? "क्या आप गरीबी रेखा के नीचे हैं? (BPL Exemption)"
                            : "BPL Cardholder Exemption"}
                        </span>
                        <span className="text-[9.5px] text-stone-500 dark:text-stone-400 block font-normal">
                          {isHi
                            ? "RTI शुल्क (₹10) पूरी तरह माफ"
                            : "No filing fee. Requires valid BPL Card."}
                        </span>
                      </div>
                    </label>
                  </div>
                ...
                <div className="space-y-1.5">
                  <div className="flex items-center justify-between">
                    <label className="block text-[10px] font-black uppercase text-stone-500 dark:text-stone-400 font-mono tracking-wider">
                      {isHi
                        ? "RTI सूचना अनुरोध (विवरण दर्ज करें) *"
                        : "Specific Information Solicited *"}
                    </label>
                    <span className="text-[9.5px] font-mono text-stone-400">
                      {query.length}/1200 chars
                    </span>
                  </div>
                  <div className="relative">
                ...
                {/* Submit and Print Actions */}
                <div className="pt-2 flex flex-col sm:flex-row gap-2.5">
                  <button
                    type="button"
                    onClick={handlePrintDraft}
                    className="flex-1 py-3 px-4 rounded-xl border border-stone-250 dark:border-stone-750 bg-stone-50 hover:bg-stone-100 dark:bg-stone-850 dark:hover:bg-stone-800 text-stone-700 dark:text-stone-200 font-display font-bold text-xs uppercase tracking-wider transition active:scale-98 flex items-center justify-center gap-1.5 cursor-pointer"
                  >
                    <Printer className="w-3.5 h-3.5 text-amber-600 dark:text-amber-500 shrink-0" />
                    <span>{isHi ? "प्रारूप प्रिंट करें (Form A)" : "Print Draft (Form A)"}</span>
                  </button>

                  <button
                    type="submit"
                    className="flex-1 py-3 px-5 rounded-xl font-display font-black text-xs uppercase tracking-wider text-neutral-900 bg-amber-500 hover:bg-amber-400 shadow-sm active:scale-98 transition flex items-center justify-center gap-1.5 cursor-pointer"
                  >
                    <span>
                      {isBplExempt
                        ? isHi
                ...
                <div className="space-y-2.5">
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    ...
                    >
                      ...
                      <span className="text-[10px] font-extrabold">
                        {isHi ? "डेबिट/क्रेडिट कार्ड" : "Card Payment"}
                      </span>
                    </button>
                  </div>
                </div>
            ...
            {step === "receipt" && receipt && (
              ...
              >
                <div className="p-4 bg-emerald-500/5 dark:bg-emerald-500/10 border border-emerald-500/20 text-emerald-950 dark:text-emerald-400 rounded-2xl flex items-start gap-3">
                  ...
                  <div>
                    <h5 className="text-xs font-black text-emerald-900 dark:text-emerald-300">
                      {isHi
                        ? "RTI सबमिशन सफलतापूर्वक दायर किया गया"
                        : "RTI Subvention Request Registered!"}
                    </h5>
                ...
                <div className="border border-stone-200 dark:border-stone-800 rounded-2xl overflow-hidden shadow-2xs font-sans text-stone-800 dark:text-stone-300">
                  <div className="p-4 bg-stone-50 dark:bg-stone-850/60 border-b border-stone-150 dark:border-stone-800 flex items-center justify-between pr-3">
                    <div className="flex items-center gap-1.5">
                    ...
                    <div className="flex items-center gap-2.5">
                      <button
                        ...
                        className="flex items-center gap-1 px-2 py-0.5 rounded text-[10px] font-bold text-amber-700 dark:text-amber-400 hover:bg-amber-100 dark:hover:bg-amber-950/40 transition cursor-pointer border border-amber-300 dark:border-amber-800/60"
                        title={isHi ? "रसीद प्रिंट करें" : "Print Acknowledgement Receipt"}
                      >
                  ...
                  <div className="p-4 space-y-3.5 bg-white dark:bg-stone-900 leading-normal text-xs divide-y divide-stone-100 dark:divide-stone-850">
                    <div className="grid grid-cols-2 gap-4 pb-1">
                      <div>
                        <span className="text-[9px] font-mono text-stone-450 uppercase block">
                    ...
                    <div className="grid grid-cols-2 gap-4 pt-3.5 leading-none">
                      ...
                      <div className="text-right">
                        ...
                        <span className="text-[10.5px] font-sans font-black text-amber-600 dark:text-amber-500 block mt-1">
                          {receipt.feesPaid > 0
                            ? `₹${receipt.feesPaid}.00 (SUCCESS)`
                            : `EXEMPTED (${receipt.bplNo})`}
                        </span>
                      </div>
                    </div>
                  </div>
                ...
                <div className="pt-2 flex flex-col sm:flex-row gap-2.5">
                  <button
                    type="button"
                    onClick={handlePrintReceipt}
                    className="flex-1 py-3 px-4 rounded-xl font-display font-black text-xs uppercase tracking-wider text-neutral-900 bg-amber-500 hover:bg-amber-400 shadow-sm active:scale-98 transition flex items-center justify-center gap-2 cursor-pointer"
                  >
                    <Printer className="w-4 h-4 text-neutral-950 shrink-0" />
                    <span>{isHi ? "आधिकारिक रसीद प्रिंट करें" : "Print Official Receipt"}</span>
                  </button>
                  ...
                  <button
                    ...
                    className="py-3 px-4 rounded-xl border border-stone-250 dark:border-stone-750 bg-white hover:bg-stone-50 dark:bg-stone-850 dark:hover:bg-stone-800 text-stone-700 dark:text-stone-200 font-display font-bold text-xs uppercase tracking-wider transition active:scale-98 flex items-center justify-center gap-1.5 cursor-pointer"
                  >
                    <FileText className="w-4 h-4 text-amber-600 dark:text-amber-500 shrink-0" />
                    <span>{isHi ? "फॉर्म 'क' प्रिंट" : "Print Form 'A'"}</span>
                  </button>
                </div>
                ...
                <div className="pt-0.5">
                  <button
                    ...
                    className="w-full py-2.5 px-4 rounded-xl font-display font-bold text-xs text-stone-600 dark:text-stone-400 hover:bg-stone-100 dark:hover:bg-stone-850 transition flex items-center justify-center gap-1.5 cursor-pointer"
                  >
                    <span>
                      {isHi ? "पंजीकरण समाप्त करें व डैशबोर्ड लौटें" : "Done / Return to Dashboard"}
                    </span>
                  </button>
                </div>
              </motion.div>
