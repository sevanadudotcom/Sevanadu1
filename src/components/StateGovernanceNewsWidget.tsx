...
export default function StateGovernanceNewsWidget({
  language,
  triggerToast,
}: StateGovernanceNewsWidgetProps) {
  const [selectedStateId, setSelectedStateId] = useState("karnataka");
  const [news, setNews] = useState<NewsItem[]>([
    {
      headline: "Karnataka Activates Unified Citizen Single-Window Systems",
      summary:
        "Administrative outposts are linked under unified e-Seva structures to accelerate demographic profile updates.",
      date: "June 2026",
      category: "Digital",
    ...
    },
    {
      ...
      summary:
        "Direct Benefit Transfer (DBT) funds are mobilized for families with linked Aadhaar identification cards.",
      date: "June 2026",
      category: "Welfare",
  ...
  const [sources, setSources] = useState<SourceItem[]>([
    { title: "Karnataka Gazette Portal", url: "https://www.india.gov.in" },
  ...
  useEffect(() => {
    ...
    const fetchStateNews = async () => {
      const stateObj = JURISDICTIONS.find((j) => j.id === selectedStateId);
      const stateName = stateObj ? stateObj.name : "Karnataka";

      setLoading(true);
      try {
        const response = await fetch(
          getApiUrl(`/api/eseva/news?state=${encodeURIComponent(stateName)}`),
          { signal: controller.signal },
        );
        if (response.ok) {
          const data = await response.json();
          if (!isCancelled && Array.isArray(data.news) && data.news.length > 0) {
            setNews(data.news);
        ...
        // Graceful handling without triggering browser console alarms
        if (!isCancelled) {
          setNews([
            {
          ...
          setSources([{ title: "National Portal of India", url: "https://www.india.gov.in" }]);
        ...
        <div className="space-y-1">
          <div className="flex items-center gap-1.5">
            ...
            <h3 className="font-extrabold text-stone-900 text-xs uppercase tracking-wider font-display flex items-center gap-1.5">
              <span>
                {language === "hi" ? "राज्य लाइव सरकारी समाचार" : "Live State Governance News"}
              </span>
      ...
      <AnimatePresence mode="wait">
        {loading ? (
          ...
          >
            ...
            <div className="space-y-1">
              <p className="text-[10px] font-mono text-stone-500">
                {language === "hi"
                  ? "आधिकारिक डेटा स्रोतों को खोजा जा रहा है..."
                  : "Filing Google Search query for live updates..."}
              </p>
          ...
          >
            ...
            {sources.length > 0 && (
              <div
                className="bg-amber-50/40 border border-amber-200/50 rounded-xl p-3.5 space-y-2 select-none"
                id="news-sources-panel"
              >
                <div className="flex items-center justify-between gap-1.5">
                  <span className="text-[8.5px] font-mono font-black tracking-widest text-stone-450 uppercase">
                    {language === "hi"
                      ? "गूगल सर्च सत्यापित स्रोत:"
                      : "Google Search Verified Citations:"}
                  </span>
                  {fallbackUsed && (
                    <span className="text-[8.5px] text-stone-400 font-sans italic">
                      Offline fallback
                    </span>
                  )}
                </div>
